...
zoom: {
      ...
      // Blockly block definition
      Blockly.Blocks['set_oled'] = {
        init: function() {
          this.appendValueInput("value")
            .setCheck(['String'])
            .appendField("laat op de ")
            .appendField(new Blockly.FieldDropdown([['linker', 'left'], ['rechter', 'right']]), 'oled')
            .appendField("OLED de ")
      ...
      // Blockly generator
      Blockly.Python['set_oled'] = function(block) {
        Blockly.Python.definitions_['import_zoef'] = 'from zoef_robot import robot\nzoef=robot.createRobot()';
        let value = Blockly.Python.valueToCode(block, 'value', Blockly.Python.ORDER_ATOMIC);
        let type = block.getFieldValue('type');
        let oled = block.getFieldValue('oled');
        let code = '';
        if (type == "text"){
           code = `zoef.setOLEDText('${oled}', ${value})\n`;
        }
        if (type == "image"){
           code = `zoef.setOLEDImage('${oled}', ${value})\n`;
        }
        if (type == "animation"){
           code = `zoef.setOLEDAnimation('${oled}', ${value})\n`;
        }
        return code;
      };
